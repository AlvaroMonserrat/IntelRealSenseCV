...
cv::Size imageSizeColorHD(640, 480);
...
void mouseEvent(int event, int x, int y, int flags, void* data)
{
	...
	switch (event)
	{
		...
		case CV_EVENT_LBUTTONUP:
			...
			*imgline = cv::Mat::zeros(imageSizeColorHD, CV_8UC3);
		break;
	}
...
int captureContourAndImageData() {
	...
	cv::Mat imgLines = cv::Mat::zeros(imageSizeColorHD, CV_8UC3);

	cv::setMouseCallback("Color Image", mouseEvent, &imgLines);
	...
	while (true)
	{
		...
		cv::Mat imageColor(imageSizeColorHD, CV_8UC3, (uint8_t*)sr300Device->get_frame_data(rs::stream::color_aligned_to_depth));
